<script lang="ts">
	...
	$: {
		const num = Number($activeResponses[id].data[component.id] ?? initialValue);
		if (isNaN(num)) {
	...
	const incrementCount = (step = 1) => {
		$activeResponses[id].data[component.id] = Math.max(
			min,
			Math.min(max, Number($activeResponses[id].data[component.id]) + step)
		);
	};
</script>
...
<div class="my-2">
	<div class="block sm:flex items-center justify-between gap-2">
		<span class="grid grid-cols-counter grid-rows-1 gap-1 mt-1 sm:mt-0">
			...
			<button on:click={() => incrementCount(-1)} class="button clickable transition-all">-</button>
			<button on:click={() => incrementCount(1)} class="button clickable transition-all">+</button>
		</span>
	</div>
	{#if error !== undefined}
